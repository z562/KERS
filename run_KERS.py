...
class TextDataset(Dataset):
    def __init__(self, file_path: str, args, block_size=512):
        ...
        if os.path.exists(cached_features_file):
            self.histories, self.responses, self.segments, self.knowledges, self.kn_vocs, self.goals = pickle.load(open(cached_features_file, "rb"))
        else:
            self.histories = []
            self.responses = []
            self.knowledges = []
            ...
            for i, sample in enumerate(x):
                ...
                self.responses.append([3] + history[-1])
            ...
            pickle.dump((self.histories, self.responses, self.segments, self.knowledges,
    ...
    def __getitem__(self, item):
        return self.histories[item], self.responses[item], self.knowledges[item], \
...
def collate(example):
    ...
    pad_goals = max([cal_max_len(g_inst) for g_inst in example[:, 5]])
    ...
    goals = [pad_data(g_inst, pad_goals) for g_inst in example[:, 5]]

    return histories, responses, kns, kn_vocs, segments, goals
...
def train(args, train_dataset, model: PreTrainedModel, tokenizer: PreTrainedTokenizer) -> Tuple[int, float]:
    ...
    for _ in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            histories, responses, knowledges, kn_vocs, segments, goals = batch
            ...
            outputs = model(input_ids=(histories, responses, knowledges, kn_vocs, goals),
