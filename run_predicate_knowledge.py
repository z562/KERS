...
class TextDataset(Dataset):
    def __init__(self, file_path: str, args, block_size=512):
        ...
        if os.path.exists(cached_features_file):
            self.histories, self.segments, self.knowledges, self.comments, self.kn_vocs, self.goals = pickle.load(open(cached_features_file, "rb"))
        else:
            self.histories = []
            self.knowledges = []
            self.comments = []
            ...
            for i, sample in enumerate(x):
                ...
                if 'test' in args.input_file.split('/')[-1]:
                    if len(history) != 0:
                ...
                # tmp_goal = tmp_goal[0] + tmp_goal[-2] + tmp_goal[-1]

                tmp_goal = self.sentence2id(goal_category[int(goals[i])])
                ...
                self.goals.append(tmp_goal + [3])

            pickle.dump((self.histories, self.segments, self.knowledges, self.comments, self.kn_vocs, self.goals), open(cached_features_file, "wb"))
    ...
    def __getitem__(self, item):
        return self.histories[item], self.knowledges[item], \
               self.comments[item], self.kn_vocs[item], \
...
def collate(example):
    ...
    pad_knowledges = max([cal_max_len(k_inst) for k_inst in example[:, 1]])
    pad_coms = max([cal_max_len(com_inst) for com_inst in example[:, 2]])
    pad_com_num = max([len(com_inst) for com_inst in example[:, 2]])
    pad_kn_vocs = max([cal_max_len(v_inst) for v_inst in example[:, 3]])
    pad_segments = max([cal_max_len(t_inst) for t_inst in example[:, 4]])
    pad_goal = max([cal_max_len(g_inst) for g_inst in example[:, 5]])

    histories = [pad_data(h_inst, pad_histories) for h_inst in example[:, 0]]
    kns = [pad_data(k_inst, pad_knowledges) for k_inst in example[:, 1]]
    coms = [pad_data(com_inst, pad_coms, pad_com_num) for com_inst in example[:, 2]]
    kn_vocs = [pad_data(v_inst, pad_kn_vocs) for v_inst in example[:, 3]]
    segments = [pad_data(t_inst, pad_segments) for t_inst in example[:, 4]]
    goals = [pad_data(g_inst, pad_goal) for g_inst in example[:, 5]]

    return histories, kns, coms, kn_vocs, segments, goals
...
def main():
    ...
    for batch in tqdm(eval_dataloader, desc="Testing"):
        histories, knowledges, comments, kn_vocs, segments, goals = batch
