...
from utils.transformer_goalgen import MultiTargetModel
import json
...
class TextDataset(Dataset):
    def __init__(self, file_path: str, args, block_size=512):
        ...
        if os.path.exists(cached_features_file):
            ...
            goal_category = ['问天气', '兴趣点推荐', '问 答', '关于明星的聊天', '寒暄', '新闻推荐', '音乐点播', '问User年龄', '问日期', '问User爱好',
                             '电影推荐', '天气信息推送', '新闻点播', '提问', '音乐推荐', '美食推荐', '再见', '播放音乐', '问User姓名', '问User性别',
                             '问时间']
            goal_category = [' '.join(list(jieba.cut(g, cut_all=False))) for g in goal_category]
            ...
            with open(os.path.join(file_path, "goal.txt"), encoding='utf-8') as f:
                truth_goal = [int(line) for line in f.readlines()]
            ...
            for i, sample in enumerate(x):
                ...
                for goal in sample['goal'].split('-->'):
                    tmp_goal.append(self.sentence2id(goal.strip()))
                tmp_goal = tmp_goal[0] + [3] + tmp_goal[-2] + [3] + tmp_goal[-1] + [3]

                tmp_res = [3] + self.sentence2id(goal_category[truth_goal[i]]) + [3]

                self.histories.append(situation + tmp)
                self.responses.append(tmp_res)
